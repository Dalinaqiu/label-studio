      ...
      <header className="mb-base">
        ...
        {webhooks.length > 0 && (
          <Typography size="small" className="text-neutral-content-subtler">
            使用 Webhook 订阅特定事件以设置集成。当事件触发时，Label Studio 会向配置的 Webhook URL 发送 HTTP POST 请求。
          </Typography>
        )}
      </header>
        ...
        {webhooks.length === 0 ? (
          <SimpleCard title="" className="bg-primary-background border-primary-border-subtler p-base">
            <EmptyState
              ...
              icon={<IconWebhook />}
              title="添加第一个 Webhook"
              description="使用 Webhook 订阅特定事件以设置集成。当事件触发时，Label Studio 会向配置的 Webhook URL 发送 HTTP POST 请求。"
              actions={
                canChangeWebhooks ? (
