const WSI_SAMPLE_PATH = "/tmp/ls-wsi-smoke.svs";
...
describe("WSI import, preview, and annotation", () => {
  beforeEach(() => {
  ...
  it("uploads an SVS file, renders the preview, and draws a region", () => {
    const email = `wsi-e2e-${Date.now()}@example.com`;
    const password = "Test123456!";
    const projectName = `WSI Smoke ${Date.now()}`;
    ...
    cy.get("#file-input").selectFile(WSI_SAMPLE_PATH, { force: true });
    ...
    cy.location("pathname").then((pathname) => {
      ...
      cy.request(`/api/tasks?project=${projectId}`).then(({ body }) => {
        ...
        cy.request(`/api/tasks/${taskId}?project=${projectId}`).then(({ body: taskBody }) => {
          ...
          expect(taskDataUrl, "task data url").to.match(/svs|tif|tiff|filepath=/i);
        ...
        cy.get(".konvajs-content", { timeout: 30000 }).should("be.visible");
